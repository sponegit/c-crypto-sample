...
*******************/
// out_len = payload_len
int aes_encrypt_ccm_v2(const BYTE payload[], WORD payload_len,
	const BYTE nonce[], unsigned short nonce_len, BYTE out[], WORD* out_len,
	BYTE mac[], WORD mac_len, const BYTE key_str[], int keysize)
{
	BYTE temp_iv[AES_BLOCK_SIZE], counter[AES_BLOCK_SIZE], * buf;
	int end_of_buf, payload_len_store_size;
	WORD key[60];
	...
	//buf = (BYTE*)malloc(payload_len /*+ assoc_len*/ +48 /*Round both payload and associated data up a block size and add an extra block.*/);  
        buf = (BYTE*)pvPortMalloc(payload_len /*+ assoc_len*/ +48 /*Round both payload and associated data up a block size and add an extra block.*/);  
	//buf = (BYTE*)pvPortMalloc(payload_len);
        if (!buf)
		return(FALSE);
	...
	// Format the first block of the formatted data.
	payload_len_store_size = AES_BLOCK_SIZE - 1 - nonce_len;
	ccm_prepare_first_format_blk(buf, 0, payload_len, payload_len_store_size, mac_len, nonce, nonce_len);
	end_of_buf = AES_BLOCK_SIZE;
	...
	// Encrypt the MAC with CTR mode with a counter starting at 0.
	aes_encrypt_ctr(mac, mac_len, mac, key, keysize, counter);
...
// plaintext_len = ciphertext_len - mac_len
// Needs a flag for whether the MAC matches.
int aes_decrypt_ccm_v2(const BYTE ciphertext[], WORD ciphertext_len,
	const BYTE nonce[], unsigned short nonce_len, BYTE plaintext[], WORD* plaintext_len,
	BYTE mac[], WORD mac_len, int* mac_auth, const BYTE key_str[], int keysize)
{
	BYTE temp_iv[AES_BLOCK_SIZE], counter[AES_BLOCK_SIZE], mac_buf[16], * buf;
	int end_of_buf, plaintext_len_store_size;
	WORD key[60];
	...
	//buf = (BYTE*)malloc(ciphertext_len /*ciphertext_len = plaintext_len */ + 48);
        buf = (BYTE*)pvPortMalloc(ciphertext_len /*ciphertext_len = plaintext_len */ + 48);
        //buf = pvPortMalloc(ciphertext_len /*ciphertext_len = plaintext_len */);
	if (!buf)
		return(FALSE);
	...
	*plaintext_len = ciphertext_len;
	...
	if (mac_auth != NULL) {
		...
		// Format the first block of the formatted data.
		plaintext_len_store_size = AES_BLOCK_SIZE - 1 - nonce_len;
		ccm_prepare_first_format_blk(buf, 0, *plaintext_len, plaintext_len_store_size, mac_len, nonce, nonce_len);
		end_of_buf = AES_BLOCK_SIZE;
